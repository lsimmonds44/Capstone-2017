...
print '' print '*** Creating TABLE COMMERCIAL_INVOICE'
GO
CREATE TABLE [dbo].[COMMERCIAL_INVOICE] (
	[COMMERCIAL_INVOICE_ID][INT]IDENTITY(10000, 1) NOT NULL			  ,
	...
	[ACTIVE]			 [BIT]					 NOT NULL DEFAULT 1   ,
	[ORDER_ID]           [INT]                   NOT NULL,
	CONSTRAINT [AK_ORDER_ID] UNIQUE([ORDER_ID] ASC)		 ,
	

	CONSTRAINT [PK_COMMERCIAL_INVOICE] PRIMARY KEY ([COMMERCIAL_INVOICE_ID] ASC)
)
GO
--print '' print '*** Creating TABLE COMMERCIAL_INVOICE_LINE'

--GO
--CREATE TABLE [dbo].[COMMERCIAL_INVOICE_LINE] (
--	[COMMERCIAL_INVOICE_ID][INT]			NOT NULL		     ,
--	[PRODUCT_ID]	 [INT]			NOT NULL			 ,
...
--
--	CONSTRAINT	[PK_COMMERCIAL_INVOICE_LINE] PRIMARY KEY([COMMERCIAL_INVOICE_ID] ASC, [PRODUCT_ID] ASC)
--)
--GO
...
GO
CREATE INDEX COMPANY_ORDER_ID ON PICKUP([COMPANY_ORDER_ID])
GO
...
GO
CREATE INDEX PICKUP_LINE_PRODUCT_ID ON PICKUP_LINE([PRODUCT_ID])
GO
...
print '' print '*** Creating TABLE SUPPLIER_PRODUCT_LOT'
GO
CREATE TABLE [dbo].[SUPPLIER_PRODUCT_LOT] (
	[SUPPLIER_PRODUCT_LOT_ID]	[INT]IDENTITY(10000, 1) NOT NULL						,
	[SUPPLIER_ID]		[INT] 					NOT NULL						,
	[PRODUCT_ID]		[INT] 					NOT NULL						,
	[QUANTITY]			[INT] 					NOT NULL						,
	[EXPIRATION_DATE]	[DATETIME] 				NOT NULL					    ,
	...
	CONSTRAINT [PK_SUPPLIER_PRODUCT_LOT] PRIMARY KEY ([SUPPLIER_PRODUCT_LOT_ID] ASC)
)
GO
CREATE INDEX SUPPLIER_PRODUCT_LOT_SUPPLIER_ID ON SUPPLIER_PRODUCT_LOT([SUPPLIER_ID])
GO
...
print '' print '*** Creating TABLE SUPPLIER_ORDER'
GO
CREATE TABLE [dbo].[SUPPLIER_ORDER] (
	...
	CONSTRAINT [PK_SUPPLIER_ORDER] PRIMARY KEY ([ORDER_ID] ASC)
)
GO
CREATE INDEX PRODUCT_ORDER_SUPPLIER_ID ON PRODUCT_ORDER([CUSTOMER_ID])
GO

print '' print '*** Creating TABLE SUPPLIER_ORDER_LINE'
CREATE TABLE [dbo].[SUPPLIER_ORDER_LINE] (
    [SUPPLIER_ORDER_LINE_ID]    INT            IDENTITY (10000, 1) NOT NULL,
    [SUPPLIER_ORDER_ID] 		INT            NOT NULL,
    [PRODUCT_ID]       			INT            NOT NULL,
    [PRODUCT_NAME]     			NVARCHAR (100) NOT NULL,
    [QUANTITY]         			INT            NOT NULL,
	[PRICE]						[DECIMAL](6,2) 
	CONSTRAINT [PK_SUPPLIER_ORDER_LINE] PRIMARY KEY CLUSTERED ([SUPPLIER_ORDER_LINE_ID] ASC),
    CONSTRAINT [fk_SUPPLIER_ORDER_LINE_SUPPLIER_ORDER_ID] FOREIGN KEY ([SUPPLIER_ORDER_ID]) REFERENCES [dbo].[SUPPLIER_ORDER] ([ORDER_ID]) ON DELETE CASCADE ON UPDATE CASCADE
)
GO

print '' print '*** Creating TABLE SUPPLIER_PRODUCT'
GO
...
print '' print '*** Creating Foreign Key COMMERCIAL_INVOICE_COMMERCIAL_ID'
GO
ALTER TABLE [dbo].[COMMERCIAL_INVOICE] WITH NOCHECK
	ADD CONSTRAINT[fk_COMMERCIAL_INVOICE_SUPPLIER_ID] FOREIGN KEY ([COMMERCIAL_ID])
	REFERENCES [dbo].[COMMERCIAL](COMMERCIAL_ID)
	ON UPDATE CASCADE
	ON DELETE CASCADE
GO

print '' print '*** Creating Foreign Key COMMERCIAL_INVOICE_LINE_COMMERCIAL_INVOICE_ID'
GO
ALTER TABLE [dbo].[COMMERCIAL_INVOICE] WITH NOCHECK
	ADD CONSTRAINT[fk_COMMERCIAL_INVOICE_ORDER_ID] FOREIGN KEY ([ORDER_ID])
	REFERENCES [dbo].[PRODUCT_ORDER](ORDER_ID)
	ON UPDATE CASCADE
	ON DELETE CASCADE
GO
...
print '' print '*** Creating Foreign Key ORDER_LINE_PRODUCT_ORDER_ID'
GO
ALTER TABLE [dbo].[ORDER_LINE] WITH NOCHECK
  ADD CONSTRAINT[fk_ORDER_LINE_PRODUCT_ORDER_ID] FOREIGN KEY ([PRODUCT_ORDER_ID])
  REFERENCES [dbo].[PRODUCT_ORDER](ORDER_ID)
  ON UPDATE CASCADE
  ON DELETE CASCADE
GO
...
print '' print '*** Creating Foreign Key PICKUP_COMPANY_ORDER_ID'
GO
ALTER TABLE [dbo].[PICKUP] WITH NOCHECK
  ADD CONSTRAINT[fk_PICKUP_COMPANY_ORDER_ID] FOREIGN KEY ([COMPANY_ORDER_ID])
  REFERENCES [dbo].[COMPANY_ORDER](COMPANY_ORDER_ID)
GO
...
print '' print '*** Creating Foreign Key PICKUP_LINE_PRODUCT_ID'
GO
ALTER TABLE [dbo].[PICKUP_LINE] WITH NOCHECK
  ADD CONSTRAINT[fk_PICKUP_LINE_PRODUCT_ID] FOREIGN KEY ([PRODUCT_ID])
  REFERENCES [dbo].[PRODUCT](PRODUCT_ID)
  ON UPDATE CASCADE
  ON DELETE CASCADE
GO
...
AS
	BEGIN
		INSERT INTO CHARITY
			(EMPLOYEE_ID, CHARITY_NAME, CONTACT_FIRST_NAME, CONTACT_LAST_NAME, PHONE_NUMBER, EMAIL, CONTACT_HOURS, STATUS)
		VALUES
			(@EMPLOYEE_ID, @CHARITY_NAME, @CONTACT_FIRST_NAME, @CONTACT_LAST_NAME, @PHONE_NUMBER, @EMAIL, @CONTACT_HOURS, @STATUS)
		RETURN @@ROWCOUNT
	END
...
AS
	BEGIN
		INSERT INTO CHARITY
			(CHARITY_NAME, CONTACT_FIRST_NAME, CONTACT_LAST_NAME, PHONE_NUMBER, EMAIL, CONTACT_HOURS)
		VALUES
			(@CHARITY_NAME, @CONTACT_FIRST_NAME, @CONTACT_LAST_NAME, @PHONE_NUMBER, @EMAIL, @CONTACT_HOURS)
		RETURN @@ROWCOUNT
	END
...
AS
	...
	END
GO
print '' print '*** Creating procedure sp_create_commercial_invoice'
GO
CREATE PROCEDURE sp_create_commercial_invoice
(
...
AS
	BEGIN
		INSERT INTO COMMERCIAL_INVOICE
			(COMMERCIAL_ID, INVOICE_DATE, SUB_TOTAL, TAX_AMOUNT, TOTAL, ORDER_ID)
		SELECT COMMERCIAL.COMMERCIAL_ID, GETDATE(), PRODUCT_ORDER.AMOUNT, PRODUCT_ORDER.AMOUNT*@TAX_RATE, PRODUCT_ORDER.AMOUNT*(1+@TAX_RATE), PRODUCT_ORDER.ORDER_ID
        FROM PRODUCT_ORDER
        INNER JOIN COMMERCIAL
        ON PRODUCT_ORDER.CUSTOMER_ID = COMMERCIAL.USER_ID
...
(
	...
	@VERIFICATION[VARBINARY](MAX)= NULL,
...
AS
	BEGIN
		INSERT INTO SUPPLIER_PRODUCT_LOT 
			(SUPPLIER_ID, PRODUCT_ID, QUANTITY, EXPIRATION_DATE)
		VALUES
			(@SUPPLIER_ID, @PRODUCT_ID, @QUANTITY, @EXPIRATION_DATE)
		RETURN @@ROWCOUNT
	END
...
AS
	BEGIN
	DECLARE @PRODUCT_ORDER_ID AS INT
	
	INSERT INTO PRODUCT_ORDER 
		(CUSTOMER_ID, EMPLOYEE_ID, ORDER_TYPE_ID, ADDRESS_TYPE, ADDRESS1, CITY, STATE, ZIP, DELIVERY_TYPE_ID, AMOUNT, ORDER_DATE, DATE_EXPECTED, DISCOUNT, ORDER_STATUS_ID, HAS_ARRIVED)
	VALUES
		(@CUSTOMER_ID, @EMPLOYEE_ID, @ORDER_TYPE_ID, @ADDRESS_TYPE, @ADDRESS1, @CITY, @STATE, @ZIP, @DELIVERY_TYPE_ID, @AMOUNT, @ORDER_DATE, @DATE_EXPECTED, @DISCOUNT, @ORDER_STATUS_ID, @HAS_ARRIVED)
...
print '' print  '*** Creating procedure sp_create_product_order_from_cart'
GO
CREATE PROCEDURE sp_create_product_order_from_cart
(
	@CUSTOMER_ID[INT],
	@ORDER_TYPE_ID[NVARCHAR](250),
	@ADDRESS_TYPE[NVARCHAR],
...
AS
	BEGIN
        IF 0<(SELECT COUNT(*) FROM USER_CART_LINE WHERE USER_ID=@CUSTOMER_ID)
        BEGIN
            DECLARE @PRODUCT_ORDER_ID AS INT
            
            INSERT INTO PRODUCT_ORDER 
                (CUSTOMER_ID, ORDER_TYPE_ID, ADDRESS_TYPE, ADDRESS1, CITY, STATE, ZIP, DELIVERY_TYPE_ID, AMOUNT, ORDER_DATE, DATE_EXPECTED, DISCOUNT, ORDER_STATUS_ID, HAS_ARRIVED)
            VALUES
                (@CUSTOMER_ID, @ORDER_TYPE_ID, @ADDRESS_TYPE, @ADDRESS1, @CITY, @STATE, @ZIP, @DELIVERY_TYPE_ID, 0, GETDATE(), @DATE_EXPECTED, @DISCOUNT, @ORDER_STATUS_ID, @HAS_ARRIVED)
            SET @PRODUCT_ORDER_ID = SCOPE_IDENTITY()
            
            INSERT INTO ORDER_LINE 
                (PRODUCT_ORDER_ID, PRODUCT_ID, PRODUCT_NAME, QUANTITY, GRADE_ID, PRICE, UNIT_DISCOUNT)
            SELECT @PRODUCT_ORDER_ID, USER_CART_LINE.PRODUCT_ID, PRODUCT.NAME, USER_CART_LINE.QUANTITY, USER_CART_LINE.GRADE_ID,
                PRODUCT_GRADE_PRICE.PRICE, 
                (CASE WHEN TOTAL_DISCOUNT.SCALED_PRODUCT_DISCOUNT<>0 AND TOTAL_DISCOUNT.SCALED_PRODUCT_DISCOUNT IS NOT NULL THEN PRODUCT_GRADE_PRICE.PRICE*TOTAL_DISCOUNT.SCALED_PRODUCT_DISCOUNT ELSE 0 END
                + CASE WHEN TOTAL_DISCOUNT.SCALED_CATEGORY_DISCOUNT<>0 AND TOTAL_DISCOUNT.SCALED_CATEGORY_DISCOUNT IS NOT NULL THEN PRODUCT_GRADE_PRICE.PRICE*TOTAL_DISCOUNT.SCALED_CATEGORY_DISCOUNT ELSE 0 END
                + CASE WHEN TOTAL_DISCOUNT.FLAT_PRODUCT_DISCOUNT IS NOT NULL THEN TOTAL_DISCOUNT.FLAT_PRODUCT_DISCOUNT ELSE 0 END + CASE WHEN TOTAL_DISCOUNT.FLAT_CATEGORY_DISCOUNT IS NOT NULL THEN TOTAL_DISCOUNT.FLAT_CATEGORY_DISCOUNT ELSE 0 END) AS UNIT_DISCOUNT
            FROM USER_CART_LINE
            INNER JOIN PRODUCT
            ...
            LEFT OUTER JOIN (
                    SELECT DEALS_FOR_PRODUCT.PRODUCT_ID, 
                ...
                ON DEALS_FOR_PRODUCT.PRODUCT_ID = DEALS_FOR_CATEGORY.PRODUCT_ID
                
                UNION
                
                SELECT DEALS_FOR_CATEGORY.PRODUCT_ID, 
                    DEALS_FOR_PRODUCT.TOTAL_AMOUNT AS FLAT_PRODUCT_DISCOUNT,
                    DEALS_FOR_PRODUCT.TOTAL_PERCENT AS SCALED_PRODUCT_DISCOUNT,
                    DEALS_FOR_CATEGORY.TOTAL_AMOUNT AS FLAT_CATEGORY_DISCOUNT,
                    DEALS_FOR_CATEGORY.TOTAL_PERCENT AS SCALED_CATEGORY_DISCOUNT
                ...
                ON USER_CART_LINE.PRODUCT_ID = TOTAL_DISCOUNT.PRODUCT_ID
            WHERE USER_CART_LINE.USER_ID = @CUSTOMER_ID
            
            DECLARE @TOTAL_AMOUNT DECIMAL(6,2)
            SELECT @TOTAL_AMOUNT = SUM((PRICE-UNIT_DISCOUNT)*QUANTITY)
            FROM ORDER_LINE
            ...
            DELETE FROM USER_CART_LINE WHERE USER_CART_LINE.USER_ID = @CUSTOMER_ID
...
AS
	BEGIN
        IF 0 < (
            SELECT CASE WHEN TOTAL_AVAILABLE IS NULL THEN 0 ELSE TOTAL_AVAILABLE END
            FROM(
                SELECT SUM(
                CASE WHEN TALLIED_PRODUCT_LOTS.TOTAL_QUANTITY IS NULL THEN 0 ELSE TALLIED_PRODUCT_LOTS.TOTAL_QUANTITY END 
                - CASE WHEN TALLIED_USER_CARTS.TOTAL_QUANTITY IS NULL THEN 0 ELSE TALLIED_USER_CARTS.TOTAL_QUANTITY END
                - CASE WHEN TALLIED_ORDERS.TOTAL_QUANTITY IS NULL THEN 0 ELSE TALLIED_ORDERS.TOTAL_QUANTITY END
                - @QUANTITY) AS TOTAL_AVAILABLE
                        FROM (SELECT PRODUCT_ID, GRADE, SUM(AVAILABLE_QUANTITY) AS TOTAL_QUANTITY FROM PRODUCT_LOT GROUP BY PRODUCT_ID, GRADE) AS TALLIED_PRODUCT_LOTS
                        LEFT OUTER JOIN (SELECT PRODUCT_ID, GRADE_ID, SUM(QUANTITY) AS TOTAL_QUANTITY FROM USER_CART_LINE GROUP BY PRODUCT_ID, GRADE_ID) AS TALLIED_USER_CARTS
                        ON TALLIED_PRODUCT_LOTS.PRODUCT_ID = TALLIED_USER_CARTS.PRODUCT_ID AND TALLIED_PRODUCT_LOTS.GRADE = TALLIED_USER_CARTS.GRADE_ID
                        LEFT OUTER JOIN (SELECT PRODUCT_ID, GRADE_ID, SUM(QUANTITY) AS TOTAL_QUANTITY FROM ORDER_LINE GROUP BY PRODUCT_ID, GRADE_ID) AS TALLIED_ORDERS
                        ON TALLIED_PRODUCT_LOTS.PRODUCT_ID = TALLIED_ORDERS.PRODUCT_ID AND TALLIED_PRODUCT_LOTS.GRADE = TALLIED_ORDERS.GRADE_ID
                WHERE TALLIED_PRODUCT_LOTS.PRODUCT_ID = @PRODUCT_ID AND TALLIED_PRODUCT_LOTS.GRADE = @GRADE_ID) AS TALLIED
        )
        BEGIN
...
print '' print  '*** Creating procedure sp_create_supplier_order'
GO
...
AS
	BEGIN
		DELETE FROM employee
		WHERE SALARY = 100 AND ACTIVE = 'True' AND DATE_OF_BIRTH = '2000-01-01' 
		RETURN @@ROWCOUNT
	END
GO
...
print '' print  '*** Creating procedure sp_delete_supplier_product_lot'
GO
CREATE PROCEDURE sp_delete_supplier_product_lot
(
	@SUPPLIER_PRODUCT_LOT_ID[INT]
)
AS
	BEGIN
		DELETE FROM supplier_product_lot
...
AS
	BEGIN
		SELECT CHARITY_ID, EMPLOYEE_ID, CHARITY_NAME, CONTACT_FIRST_NAME, CONTACT_LAST_NAME, PHONE_NUMBER, EMAIL, CONTACT_HOURS, STATUS
		FROM charity
		WHERE CHARITY_ID = @CHARITY_ID
	END
GO
...
AS
	BEGIN 
		SELECT COMMERCIAL_INVOICE_ID, COMMERCIAL_ID, INVOICE_DATE, SUB_TOTAL, TAX_AMOUNT, TOTAL, AMOUNT_PAID, APPROVED, ACTIVE, ORDER_ID
...
print '' print '*** Creating procedure sp_retrieve_commercial_invoice_list_by_user_name'
GO
CREATE PROCEDURE sp_retrieve_commercial_invoice_list_by_user_name
(
	@USER_NAME [NVARCHAR](50)
)
AS
	BEGIN
		SELECT COMMERCIAL_INVOICE_ID, COMMERCIAL_INVOICE.COMMERCIAL_ID, INVOICE_DATE, SUB_TOTAL, TAX_AMOUNT, TOTAL, AMOUNT_PAID, APPROVED, COMMERCIAL_INVOICE.ACTIVE, COMMERCIAL_INVOICE.ORDER_ID
		FROM COMMERCIAL_INVOICE
        INNER JOIN COMMERCIAL
            ON COMMERCIAL.COMMERCIAL_ID = COMMERCIAL_INVOICE.COMMERCIAL_ID
        INNER JOIN APP_USER
            ON APP_USER.USER_ID = COMMERCIAL.USER_ID
		WHERE APP_USER.USER_NAME = @USER_NAME
	END
GO

print '' print '*** Creating procedure sp_retrieve_commercial_invoice_lines_by_commercial_invoice_id'
GO
CREATE PROCEDURE sp_retrieve_commercial_invoice_lines_by_commercial_invoice_id
(
	@COMMERCIAL_INVOICE_ID[INT]
)
AS
	BEGIN
		SELECT COMMERCIAL_INVOICE_ID, ORDER_LINE.PRODUCT_ID, ORDER_LINE.QUANTITY, ORDER_LINE.PRICE, ORDER_LINE.UNIT_DISCOUNT, PRODUCT.NAME
		FROM COMMERCIAL_INVOICE
		INNER JOIN ORDER_LINE
			ON ORDER_LINE.PRODUCT_ORDER_ID = COMMERCIAL_INVOICE.ORDER_ID
        INNER JOIN PRODUCT
...
AS
	BEGIN
		SELECT PICKUP_ID, SUPPLIER_ID, WAREHOUSE_ID, DRIVER_ID, EMPLOYEE_ID, COMPANY_ORDER_ID
		FROM pickup
		WHERE PICKUP_ID = @PICKUP_ID
	END
GO
...
AS
	BEGIN
		SELECT PICKUP_LINE_ID, PICKUP_ID, PRODUCT_ID, QUANTITY, PICK_UP_STATUS
		FROM pickup_line
		WHERE PICKUP_LINE_ID = @PICKUP_LINE_ID
	END
GO
...
print '' print  '*** Creating procedure sp_retrieve_pickup_line_pickups_receieved'
GO
CREATE PROCEDURE sp_retrieve_pickup_line_pickups_receieved
(
	@PICK_UP_STATUS[BIT]
)
AS
	BEGIN
		SELECT PICKUP_LINE_ID, PICKUP_ID, PICKUP_LINE.PRODUCT_ID, QUANTITY, PICK_UP_STATUS, PRODUCT.NAME
		FROM PICKUP_LINE
        INNER JOIN PRODUCT
            ON PRODUCT.PRODUCT_ID = PICKUP_LINE.PRODUCT_ID
		WHERE (PICKUP_LINE.PICK_UP_STATUS=@PICK_UP_STATUS)
	END
GO
...
print '' print  '*** Creating procedure sp_retrieve_supplier_product'
GO
CREATE PROCEDURE sp_retrieve_supplier_product
(
	@SUPPLIER_ID[INT]
)
AS
	BEGIN
		SELECT NAME, PRODUCT.PRODUCT_ID, UNIT_PRICE
		FROM product, supplier_product
		WHERE SUPPLIER_ID = @SUPPLIER_ID
	END
GO
...
AS
	BEGIN
		...
		FROM Product pd
		INNER JOIN Product_Lot pl ON (pl.product_id = pd.product_id)
		INNER JOIN Supplier sp ON (sp.supplier_id = pl.supplier_id)
		INNER JOIN Product_Grade_Price pgp ON (pgp.product_id = pd.product_id)
		INNER JOIN Product_Category pc ON (pc.product_id = pd.product_id)
		WHERE pd.active = 1
	END
GO
...
(
	...
	@VERIFICATION[VARBINARY](MAX)=NULL,@VERIFICATION_ESCAPE[BIT] = NULL,
...
AS
	BEGIN
		...
		AND (PICKUP.COMPANY_ORDER_ID=@COMPANY_ORDER_ID OR @COMPANY_ORDER_ID IS NULL)
	END
GO
...
AS
	BEGIN
		...
		AND (PICKUP_LINE.PRODUCT_ID=@PRODUCT_ID OR @PRODUCT_ID IS NULL)
...
print '' print  '*** Creating procedure sp_retrieve_supplier_order_line'
GO
CREATE PROCEDURE sp_retrieve_supplier_order_line
(
	@SUPPLIER_ORDER_ID[INT]
)
AS
BEGIN
    SELECT SUPPLIER_ORDER_LINE_ID, SUPPLIER_ORDER_ID, PRODUCT_ID, PRODUCT_NAME, PRICE,
			 QUANTITY
...
(
	...
	@new_ADDRESS2[NVARCHAR](100)=null,
...
AS
	BEGIN
		UPDATE app_user
		SET FIRST_NAME = @new_FIRST_NAME, LAST_NAME = @new_LAST_NAME, PHONE = @new_PHONE, E_MAIL_ADDRESS = @new_E_MAIL_ADDRESS, E_MAIL_PREFERENCES = @new_E_MAIL_PREFERENCES, USER_NAME = @new_USER_NAME, ACTIVE = @new_ACTIVE, ADDRESS1 = @new_ADDRESS1, ADDRESS2 = @new_ADDRESS2, CITY = @new_CITY, STATE = @new_STATE, ZIP = @new_ZIP
		WHERE (USER_ID = @USER_ID)
...
AS
	BEGIN
		UPDATE charity
		SET EMPLOYEE_ID = @new_EMPLOYEE_ID, CHARITY_NAME = @new_CHARITY_NAME, CONTACT_FIRST_NAME = @new_CONTACT_FIRST_NAME, CONTACT_LAST_NAME = @new_CONTACT_LAST_NAME, PHONE_NUMBER = @new_PHONE_NUMBER, EMAIL = @new_EMAIL, CONTACT_HOURS = @new_CONTACT_HOURS, STATUS = @new_STATUS
...
(
	...
	@old_VERIFICATION[VARBINARY](MAX)=null,
	@new_VERIFICATION[VARBINARY](MAX),
...
(
	...
	@new_EMPLOYEE_ID[INT],
	@old_COMPANY_ORDER_ID[INT],
	@new_COMPANY_ORDER_ID[INT]
)
AS
	BEGIN
		UPDATE pickup
		SET SUPPLIER_ID = @new_SUPPLIER_ID, WAREHOUSE_ID = @new_WAREHOUSE_ID, DRIVER_ID = @new_DRIVER_ID, EMPLOYEE_ID = @new_EMPLOYEE_ID, COMPANY_ORDER_ID = @new_COMPANY_ORDER_ID
		...
		AND (COMPANY_ORDER_ID = @old_COMPANY_ORDER_ID)
		RETURN @@ROWCOUNT
	END
GO
...
AS
	BEGIN
		UPDATE pickup_line
		SET PICKUP_ID = @new_PICKUP_ID, PRODUCT_ID = @new_PRODUCT_ID, QUANTITY = @new_QUANTITY, PICK_UP_STATUS = @new_PICK_UP_STATUS
...
AS
	BEGIN
		SELECT APP_USER.USER_ID, SUPPLIER.FARM_ADDRESS, "", SUPPLIER.FARM_CITY, SUPPLIER.FARM_STATE, ""
		FROM APP_USER, SUPPLIER
...
AS
	BEGIN
		SELECT APP_USER.USER_ID, PRODUCT_ORDER.ADDRESS1, PRODUCT_ORDER.ADDRESS2, PRODUCT_ORDER.CITY, PRODUCT_ORDER.STATE, PRODUCT_ORDER.ZIP
		FROM APP_USER, CUSTOMER, DELIVERY, PRODUCT_ORDER
		WHERE APP_USER.USER_ID = CUSTOMER.CUSTOMER_ID
		AND CUSTOMER.CUSTOMER_ID = PRODUCT_ORDER.CUSTOMER_ID
...
print '' print  '*** Creating procedure sp_save_order'
GO
...
BEGIN
	...
	INSERT INTO PRODUCT_ORDER 
		(CUSTOMER_ID, ORDER_TYPE_ID, AMOUNT, ORDER_DATE, DATE_EXPECTED, DISCOUNT, ORDER_STATUS_ID, USER_ADDRESS_ID, HAS_ARRIVED)
	SELECT CUSTOMER_ID, ORDER_TYPE_ID, AMOUNT, ORDER_DATE, DATE_EXPECTED, DISCOUNT, 'Saved', USER_ADDRESS_ID, HAS_ARRIVED
	FROM PRODUCT_ORDER
	WHERE ORDER_ID = @ORDER_ID
	...
	SELECT @new_order_id, PRODUCT_ID, PRODUCT_NAME, QUANTITY, GRADE_ID, PRICE, UNIT_DISCOUNT
	FROM ORDER_LINE
	WHERE ORDER_LINE.PRODUCT_ORDER_ID = @ORDER_ID
	
END
GO


print '' print  '*** Creating procedure sp_load_order'
GO
CREATE PROCEDURE sp_load_order
(
	@ORDER_ID[INT]
)
AS
BEGIN
    ...
    SELECT @userID = CUSTOMER_ID
    FROM PRODUCT_ORDER
    WHERE ORDER_ID = @ORDER_ID
	
	DECLARE LINE_CURSOR CURSOR FOR
    SELECT PRODUCT_ID, QUANTITY, GRADE_ID
    FROM ORDER_LINE
    WHERE PRODUCT_ORDER_ID = @ORDER_ID
    OPEN LINE_CURSOR
    
    FETCH NEXT FROM LINE_CURSOR INTO @PRODUCT_ID, @QUANTITY, @GRADE_ID
    
    WHILE @@fetch_status=0
    BEGIN
        EXEC sp_add_to_cart @PRODUCT_ID, @GRADE_ID, @QUANTITY, @userID
    FETCH NEXT FROM LINE_CURSOR INTO @PRODUCT_ID, @QUANTITY, @GRADE_ID
    END
...
print '' print  '*** Creating procedure sp_saved_orders_by_user'
GO
CREATE PROCEDURE sp_saved_orders_by_user
(
	@USER_NAME[NVARCHAR](50)
)
AS
BEGIN
    ...
    FROM PRODUCT_ORDER
    INNER JOIN APP_USER
    ON PRODUCT_ORDER.CUSTOMER_ID = APP_USER.USER_ID
    WHERE APP_USER.USER_NAME = @USER_NAME
    AND PRODUCT_ORDER.ORDER_STATUS_ID = 'Saved'
	
END

print '' print  '*** Creating procedure sp_update_company_order_has_arrived'
GO
CREATE PROCEDURE sp_update_company_order_has_arrived
(
	@COMPANY_ORDER_ID[INT],
	@old_HAS_ARRIVED [BIT],
	@new_HAS_ARRIVED [BIT]
