...
GO
ALTER TABLE [dbo].[COMMERCIAL_INVOICE_LINE] WITH NOCHECK
	ADD CONSTRAINT[fk_COMMERCIAL_INVOICE_LINE_COMMERCIAL_INVOICE_ID] FOREIGN KEY ([COMMERCIAL_INVOICE_ID])
	REFERENCES [dbo].[COMMERCIAL_INVOICE](COMMERCIAL_INVOICE_ID)
	ON UPDATE CASCADE
	ON DELETE CASCADE
GO

print '' print '*** Creating Foreign Key COMMERCIAL_INVOICE_LINE_PRODUCT_LOT_ID'
GO
ALTER TABLE [dbo].[COMMERCIAL_INVOICE_LINE] WITH NOCHECK
	ADD CONSTRAINT[fk_COMMERCIAL_INVOICE_LINE_PRODUCT_ID] FOREIGN KEY ([PRODUCT_ID])
	REFERENCES [dbo].[PRODUCT](PRODUCT_ID)
	ON UPDATE CASCADE
	ON DELETE CASCADE
GO
...
AS
	BEGIN
		INSERT INTO COMMERCIAL_INVOICE
			(COMMERCIAL_ID, INVOICE_DATE, SUB_TOTAL, TAX_AMOUNT, TOTAL, AMOUNT_PAID)
		VALUES
			(@COMMERCIAL_ID, @INVOICE_DATE, @SUB_TOTAL, @TAX_AMOUNT, @TOTAL, @AMOUNT_PAID)
		SELECT SCOPE_IDENTITY()
	END
...
print '' print '*** Creating procedure sp_create_commercial_invoice_line'
GO
CREATE PROCEDURE sp_create_commercial_invoice_line
(
...
AS
	BEGIN
		INSERT INTO COMMERCIAL_INVOICE_LINE
			(COMMERCIAL_INVOICE_ID, PRODUCT_ID, QUANTITY_SOLD, PRICE_EACH, ITEM_DISCOUNT, ITEM_TOTAL)
		VALUES
			(@COMMERCIAL_INVOICE_ID, @PRODUCT_ID, @QUANTITY_SOLD, @PRICE_EACH, @ITEM_DISCOUNT, @ITEM_TOTAL)
		RETURN @@ROWCOUNT
	END
...
AS
	BEGIN
		SELECT COMMERCIAL_INVOICE_ID, COMMERCIAL_INVOICE_LINE.PRODUCT_ID, QUANTITY_SOLD, PRICE_EACH, ITEM_DISCOUNT, ITEM_TOTAL, PRODUCT.NAME
		FROM COMMERCIAL_INVOICE_LINE
        INNER JOIN PRODUCT
            ON COMMERCIAL_INVOICE_LINE.PRODUCT_ID = PRODUCT.PRODUCT_ID
		WHERE COMMERCIAL_INVOICE_ID = @COMMERCIAL_INVOICE_ID
	END
GO
